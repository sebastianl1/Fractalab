...
**ChaosLab STEM: Bifurcación ↔ Mandelbrot Explorer** (FractaLab Sl).
...
Todos heredan de `BifurcationModel` (`BaseModel.ts`), que define:
...
1. **LogisticModel** — `x_{n+1} = r·x·(1−x)` — isomorfismo **exacto** con el eje real de Mandelbrot.
...
│   ├── mandelbrotCompute.ts # Cómputo CPU puro de Mandelbrot (fallback)
...
│   └── mandelbrot.worker.ts # Fallback CPU de Mandelbrot en worker
├── components/
│   ├── BifurcationCanvas.ts # Diagrama cacheado + Lyapunov (worker)
│   ├── MandelbrotShader.ts  # WebGL (Three.js) + fallback worker
...
- El fallback CPU de Mandelbrot corre en un **Web Worker** (resolución
...
2. **Mandelbrot como Atlas**: el corte real `c ∈ [−2, 0.25]` cartografía las
